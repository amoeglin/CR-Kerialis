...
namespace CompteResultat.BL
{
    public class BLCompteResultat
    {
        ...
        //only required for Auto CR
        public int CRAutoId = -1;
        public string ListName { get; set; }
        public string LevelGrEnt { get; set; }

        public Groups CRGroups { get; set; }
        public Entrepr CREntrepr { get; set; }
        ...
        {            
            try
            {
                ...
                bool autoReport = IsAutoGenerated.HasValue ? IsAutoGenerated.Value : false;
                if (autoReport)
                {
                    CRExists = false;

                    //add CRAutoMain record                    
                    string reportType = IsPrev ? "PREV" : "SANTE";
                    string mainFolder = Path.GetFileName(ExcelCRPath);

                    if (CRAutoId == -1)
                    {
                        CRAutoId = CRAutoMain.Insert(
                            new CRAutoMain { ListName = ListName, ReporType = reportType, MainFolderName = mainFolder, 
                                UserName = UserName, CreationDateTime = DateTime.Now });
                    }

                    Id = AddDBRecord();
                    //Id = CRAutoId;
                } 
                else if (!modeSynthese && !autoReport)
                {
                    if (Id != C.cINVALIDID)
                    {
                        CRExists = true;
        ...
        private void CreateExcelDoc(int CRId, CRPlanning crp, bool modeSynthese)
        {
            try
            {
                ...
                if (!Directory.Exists(ExcelCRPath)) Directory.CreateDirectory(ExcelCRPath);
                ...
                //Delete PrevProv
                if (File.Exists(newPrevProvPath))
                    File.Delete(newPrevProvPath);
            }
            catch (Exception ex)
            {
            ...
            int yearsToCalc, bool calculateProvision)
        {
            ...
            try
            {
                ...
                if (autoReport)
                {
                    ExcelSheetHandler.FillIsGroup(fiExcelFile, LevelGrEnt);
                    if (LevelGrEnt == "Group")
                    {
                        ExcelSheetHandler.FillCRGroups(fiExcelFile, CRGroups);
                    } else
                    {
                        ExcelSheetHandler.FillCREntrepr(fiExcelFile, CREntrepr);
                    }
                }                

                ExcelSheetHandler.FillFraisPrev(fiExcelFile);
        ...
        public void CreateExcelSANTEData(FileInfo fiExcelFile, string assurNameList, string parentCompanyNameList, string companyNameList, string contrNameList, DateTime debutPeriod, DateTime finPeriod, 
            int yearsToCalc, string college, DateTime dateArret, CRPlanning crp, string crFilePathPPT, string crFilePath, C.eReportTemplateTypes templateType, 
            bool reportWithOption, bool calculateProvision)
        {
            DateTime debutNew;
            DateTime finNew;
            ...
            {                
                ...
                ExcelSheetHandler.FillAffichageSheet(fiExcelFile, assurList[0]);
                ExcelSheetHandler.FillFraisSante(fiExcelFile);
        ...
        private void MapDALObject()
        {
            ...
            if (autoReport)
            {
                CRDALObject.LevelGrEnt = LevelGrEnt;
                CRDALObject.CRAutoId = CRAutoId;
                if (LevelGrEnt != "Group")
                {
                    CRDALObject.RaisonSociale = CREntrepr.RaisonSociale;
                    CRDALObject.StructureCotisation = CREntrepr.Structure;  
                }                    
            }
        }
