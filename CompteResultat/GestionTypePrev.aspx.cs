...
{
    public partial class GestionTypePrev : System.Web.UI.Page
    {
                ...
                {                                  
                    ...
                    if (uploadExcel.PostedFile != null)
                    {
                        if (uploadExcel.PostedFile.FileName.Length > 0)
                        {
                            ...
                            BLTypePrev.ImportTypePrev(fullUploadPath, true);                            
                        }
                    }                                   
                }
            }
        ...
        public IEnumerable<TypePrevoyance> GetTP()
        {
            try
            {
                return TypePrevoyance.GetTypePrev();
            }
            catch (Exception ex) { UICommon.HandlePageError(ex, this.Page, "GestionTypePrev::GetTP"); return null; }
        }
        ...
        protected void cmdExport_Click(object sender, EventArgs e)
        {
            ...
            try
            { 
                ExcelPackage pack = BLTypePrev.ExportTypePrev();
                
                uploadPath = Path.Combine(Request.PhysicalApplicationPath, C.uploadFolder);
                uploadPath = Path.Combine(uploadPath, User.Identity.Name + "_TypePrevoyance.xlsx");
