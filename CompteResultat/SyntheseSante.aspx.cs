...
{
    public partial class SyntheseSante : System.Web.UI.Page
    {
        ...
        {
            //### replace with correct value from option
        ...
        protected void cmdcreate_Click(object sender, EventArgs e)
        {
            ...
            // GLOBAL SOCIETE PRODUIT => get all assureurs that end with _PRODUCT => get all Comps & Subsids for those Assur
            if (chkGlobalProd.Checked)
            {
                //User.Identity.Name;                
                reportName = "_GLOBAL_SOCIETE_PRODUIT_" + dateTimeToday;
                C.eReportTypes repType = C.eReportTypes.GlobalEnt;
                C.eReportTemplateTypes repTemplate = C.eReportTemplateTypes.SANTE_GLOBAL;                
                assurNames = Assureur.GetEnterpriseAssNamesByType(C.cASSTYPEPRODUCT);                
            ...
            {               
                reportName = "_GLOBAL_SOCIETE_ENTREPRISE_" + dateTimeToday;
                C.eReportTypes repType = C.eReportTypes.GlobalEnt;
                C.eReportTemplateTypes repTemplate = C.eReportTemplateTypes.SANTE_GLOBAL;
                assurNames = Assureur.GetEnterpriseAssNamesByType(C.cASSTYPEENTERPRISE);
            ...
            // STANDARD TOUS PRODUITS => get all assureurs that end with _PRODUIT => get all Comps & Subsids for those Assur
            if (chk2AnsAllProd.Checked)
            {
                reportName = "_STANDARD_TOUS_PRODUITS_" + dateTimeToday;
                C.eReportTypes repType = C.eReportTypes.Standard;
                C.eReportTemplateTypes repTemplate = C.eReportTemplateTypes.SANTE;
                assurNames = Assureur.GetEnterpriseAssNamesByType(C.cASSTYPEPRODUCT);
            ...
            // STANDARD CHAQUE PRODUITS => get all assureurs that end with _PRODUIT => get all Comps & Subsids for those Assur
            if (chk2AnsEachProd.Checked)
            {                
                C.eReportTypes repType = C.eReportTypes.Standard;
                C.eReportTemplateTypes repTemplate = C.eReportTemplateTypes.SANTE;
                List<string> allAssur = Assureur.GetEnterpriseAssNamesByType(C.cASSTYPEPRODUCT);
 
                foreach (string ass in allAssur)
                {
                    assurNames = new List<string>();
                    assurNames.Add(ass);

                    int myassurId = Assureur.GetAssIdForAssName(ass);
                    List<string> assurIds = new List<string>();
                    assurIds.Add(myassurId.ToString());                    
